...
- [ ] Global `npm install` pinning: npm cannot digest-pin a global install, so the practical form is exact-version pinning — already in place (`turbo@2.10.12`, `pnpm@12.4.1`, `prisma@7.10.0`). Verify each global install line carries an exact version; drop any that lost its runner-stage purpose after the slim-down attempt. Note the exact-version decision in Implementation Notes (and as the settled answer to the deferred item).
- [ ] Un-hardcode PORT: runner `ENV PORT=3000` (EXPOSE stays 3000 — it's documentation; Render overrides via env and the app already obeys).

**T2 — Web Docker image** (`apps/web/Dockerfile` NEW, `apps/web/nginx/default.conf.template` NEW or inline)

- [ ] Multi-stage: pruner (`turbo prune @locker/web`) → build (`pnpm install --frozen-lockfile`, `turbo run build --filter=@locker/web` with `ARG VITE_API_BASE_URL=''` passed as build env) → runner `nginx:1.27-alpine`.
- [ ] nginx config: serve the SPA from `/usr/share/nginx/html`; `location / { try_files $uri /index.html; }` so deep links (`/retrieve`, `/agent`) return the app, never 404; gzip on for js/css/json; listen on `${PORT}` via the templates/envsubst mechanism with a default of 80.
...
- [ ] `@fastify/cors` registered **only when** `CORS_ORIGIN` is set: comma-separated origin list, `origin` as an allow-list, `methods: ['GET','POST']`. Unset → plugin not registered → local dev (Vite proxy, same-origin) byte-identical to today.
- [ ] `parseEnv`: optional `CORS_ORIGIN`; when present, non-empty after trim, each comma-part a plausible origin (`^https?://`). Boot error follows the existing one-clear-error-naming-the-variable pattern.
- [ ] Tests: allowed origin → `access-control-allow-origin` echoed; unlisted origin → no ACAO header; unset env → no CORS headers on any route (proves dev-unchanged). Integration style via `app.inject()` with the env set per-app-instance, same as existing env-dependent suites.

**T4 — compose: the full local stack** (`docker-compose.yml`)

- [ ] Add `web` service: build with `VITE_API_BASE_URL=http://localhost:3000`, ports `8080:80`, `depends_on: api: condition: service_healthy`, own healthcheck (wget/curl against `/` — alpine nginx has wget via busybox).
- [ ] Fold-in (container-ops bundle): `restart: unless-stopped` on api + web (postgres keeps default behavior — it's a dev service with a named volume); `start_period` on the api healthcheck (15s covers cold migrate-on-boot); guard the migrate step in the api CMD: `test -n "$DATABASE_URL" || { echo 'DATABASE_URL is not set' >&2; exit 1; } && prisma migrate deploy && node dist/index.js`.
- [ ] Compose still supports the dev shape untouched: `docker compose up -d postgres` + host-run dev servers.
...
- [ ] Comment header: free-tier spin-down expected; slug-collision runbook note.
...
- [ ] `apps/web/src/store-package.test.tsx` fixture `clx8m2qk4` → a 6-char AD-9 id (`M4XT2B`) — last cuid-shaped string in the repo's tests.
- [ ] Check `turbo run build` emits no missing-declared-output warnings for `@locker/web` (4.x may have fixed this; if yes, tick the deferred item as already-done in `deferred-work.md`).
...
- [ ] Full pipeline at boundary: `npx pnpm@12.4.1 turbo run build test lint` (one run, green).
- [ ] Update `deferred-work.md`: mark the container-ops bundle + Dockerfile-pinning items absorbed by 5.1 (with what was actually done).
